...
public class BigXmlTestCursorApi {
	...
	public void startSplitting() throws Exception {
		XMLStreamReader2 xmlStreamReader = ((XMLInputFactory2) XMLInputFactory
				.newInstance()).createXMLStreamReader(BigXmlTestCursorApi.class
				.getResource("/BigXmlTest.xml"));
		PrintWriter validationResults = enableValidationHandling(xmlStreamReader);

		int fileNumber = 0;
		int dataRepetitions = 0;
		XMLStreamWriter xmlStreamWriter = openOutputFileAndWriteHeader(++fileNumber); // Prepare
																						// first
																						// file
		...
		try {
			while (xmlStreamReader.hasNext()) {
				...
				if (xmlStreamReader.getEventType() == XMLEvent.START_ELEMENT
					...
					if (dataRepetitions != 0 && dataRepetitions % 2 == 0) {
						xmlStreamWriter.close();
						xmlStreamWriter = openOutputFileAndWriteHeader(++fileNumber);
						dataRepetitions = 0;
					}
					transformer
							.transform(
									...
									new StAXResult(
											new FragmentXMLStreamWriterWrapper(
													new AvoidDefaultNsPrefixStreamWriterWrapper(
															xmlStreamWriter,
															DOCUMENT_NS))));
					dataRepetitions++;
				}
	...
	private XMLStreamWriter openOutputFileAndWriteHeader(int fileNumber)
			throws Exception {
		XMLOutputFactory xmlOutputFactory = XMLOutputFactory.newInstance();
		xmlOutputFactory.setProperty(XMLOutputFactory.IS_REPAIRING_NAMESPACES,
				true);
		XMLStreamWriter writer = xmlOutputFactory
				.createXMLStreamWriter(new FileOutputStream(new File(
	...
	private PrintWriter enableValidationHandling(
			XMLStreamReader2 xmlStreamReader) throws XMLStreamException,
			IOException {
		...
		XMLValidationSchema xmlValidationSchema = xmlValidationSchemaFactory
				.createSchema(BigXmlTestCursorApi.class
						.getResource("/BigXmlTest.xsd"));

		final PrintWriter validationResults = new PrintWriter(
				new BufferedWriter(new FileWriter(new File(
						System.getProperty("java.io.tmpdir"),
						"ValidationResults.txt"))));

		xmlStreamReader
				.setValidationProblemHandler(new ValidationProblemHandler() {
					@Override
					public void reportProblem(
							XMLValidationProblem validationError)
